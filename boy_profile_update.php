...
if(isset($_POST['update_profile'])){
   ...
   $update_profile->execute([$name, $email, $mobile, $deliveryboy_id]);
/*
...
*/
   ...
   if(!empty($update_pass) AND !empty($new_pass) AND !empty($confirm_pass)){
      if($update_pass != $old_pass){
         ...
         $update_pass_query = $conn->prepare("UPDATE `deliveryboy` SET password = ? WHERE id = ?");
         $update_pass_query->execute([$confirm_pass, $deliveryboy_id]);
...
<head>
   ...
   <title>update BoY profile</title>
...
--->      <i class="far fa-smile" style="font-size:100px"></i>
      ...
      <div class="flex-btn">
         ...
         <a href="dhome.php" class="option-btn">Go back</a>
      </div>
   </form>
